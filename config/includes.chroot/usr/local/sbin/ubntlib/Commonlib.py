...
class ExpttyProcess():
    ...
    def expect2act(self, timeout, exptxt, action=None, rt_buf=None):
    ...
    def expect2actnrd(self, timeout, prompt, action, rt_buf):
        self.proc.logfile_read = sys.stdout

        # expect last time command buffer
        index = self.proc.expect([prompt, pexpect.EOF, pexpect.TIMEOUT], timeout)
        if(index == 1):
            self.log.error("[ERROR:EOF]: Expect \"" + prompt + "\"")
            return -1
        if(index == 2):
            self.log.error("[ERROR:Timeout]: Expect \"" + prompt + "\" more than " + str(timeout) + " seconds")
            return -1
        ...
        rdbuf = self.proc.before
        rdbuf = rdbuf.strip(' ')
        rdbuf = rdbuf.split('\r\n')
        for e in rdbuf:
            if(e != ''):
                rt_buf.append(e.strip(' '))
            

        # only send newline for getting hint
        self.proc.send(self.newline)
        return 0
