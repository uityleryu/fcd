...
class SnxxLib:
    ...
    def prepare_cmd(self, cmd = "\rback\rsetlvl 0", timeout = CMD_TOUT_PREPARE):
    ...
    def __send_cmd_helper(self, menu_level: str, cmd: str, answer_lines_cnts, timeout):
        ...
        while True:
            ...
            if line:
                if wait_echo == 1:
                    if not line == cmd:
                        return ["__send_cmd_helper: wrong echo responce '{0}'".format(line), []]
                ...
                if line == greeting_main:
                    if not greeting == greeting_main:
                        return ["__send_cmd_helper: menu '{0}' is absent (received response menu level '{1}', requested '{0}')".format(menu_level, menu_level_main), []]
            ...
            if  time.time() > end_ts:
                return ["__send_cmd_helper: too much data is read from the serial port during {:0.0f} ms".format((time.time() - start_ts) * 1000), []]
        ...
        if not len(answer_lines_cnts) == 0 and found == 0:
            ...
            return ["__send_cmd_helper: answer lines count {0} is not in range {1}: {2}".format(lines_count, str(answer_lines_cnts), error_str), []]
    ...
    def __get_devreg_table_size_int_helper(self):
        ...
        [error_str, table_cnt_str] = self.__send_cmd_regex_helper("devreg", "table size", pattern, self.CMD_TOUT_DEFAULT)
        if error_str:
            return [error_str, ""]
    ...
    def __get_devreg_flash_size_int_helper(self):
        ...
        [error_str, flash_size_str] = self.__send_cmd_regex_helper("devreg", "flash size", pattern, self.CMD_TOUT_DEFAULT)
        if error_str:
            return [error_str, ""]
    ...
    def read_devreg_helper_and_flash_data(self, helper_text_file: str, flash_binary_file: str):
        ...
        while True:
            ...
            if not file_crc == mem_crc:
                error_str = "read_devreg_helper_and_flash_data: calculated crc ({0}) and requested crc ({1}) are not the same".format(file_crc, mem_crc)
                break
    ...
    def write_devreg_flash_data(self, flash_binary_file: str):
        ...
        if file_size == 0:
            return "write_defreg_flash_data: write_defreg_flash_data: file '{0}' is empty".format(basename(flash_binary_file))
        ...
        while True:
            ...
            if file_size > flash_size:
                error_str = "write_defreg_flash_data: wile size ({0}) is greater than flash size ({1})".format(file_size, flash_size)
                break
            ...
            if not file_crc == mem_crc:
                error_str = "write_defreg_flash_data: calculated crc ({0}) and requested crc ({1}) are not the same".format(file_crc, mem_crc)
                break
