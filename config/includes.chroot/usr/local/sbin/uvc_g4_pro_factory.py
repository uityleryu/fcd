...
class USUDCALPINEFactoryGeneral(ScriptBase):
    def __init__(self):
        ...
        self.password = "ubnt"
        self.polling_mins = 5
        self.flash_module = "m25p80.ko"
        self.helperexe = "helper_S5L"
        self.firmware = "ubnt-aircam-fw.bin"
        self.host_toolsdir_dedicated = os.path.join(self.tftpdir, self.tools, "uvc")
        self.fw_path = os.path.join(self.tftpdir, "uvc-fw", self.firmware)
    ...
    def upload_falsh_module(self):

        flash_module_path = os.path.join(self.host_toolsdir_dedicated, self.flash_module)

        cmd_grep = "lsmod | grep {}".format(self.flash_module.split(".")[0])
    ...
    def registration(self, qrcode=True):
        ...
        if self.fcd_id == "" or self.sem_ver == "" or self.sw_id == "" or self.fw_ver == "":
            ...
            # cameras don't have qrcode
            if qrcode is True:
                regparam.append("-i field=qr_code,format=hex,value=" + self.qrhex)
    ...
    def run(self):
        ...
        log_debug("Waiting - PLUG in the device...")
        # log_debug("Waiting for device start...")
        # log_debug("Checking DEV name")

        log_debug("Uploading flash module...")

        self.upload_falsh_module()
        ...
        if REGISTER_EN is True:
            self.registration(qrcode = False)
