...
(parted -s $DEV mkpart primary fat32 1049k 150M \
 && parted -s $DEV mkpart primary ext2 150M 3800M) \
    || fatal 'Failed to create partitions'

(mkdosfs ${DEV}p1 && mkfs.ext3 ${DEV}p2) || fatal 'Make filesystems'

D1=/media/mmc1
D2=/media/mmc2
mkdir -p $D1 $D2 || fatal 'Create directories'

(mount ${DEV}p1 $D1 && mount ${DEV}p2 $D2) || fatal 'Mount filesystems'
...
(
    cd $D1
    (tar -xf $IMG_FILE $TKERN $TKERN.md5 && sync) || fatal 'Extract kernel'
    osum=$(cat $TKERN.md5)
...
(
    cd $D2
    (tar -xf $IMG_FILE $TRIMG $TRIMG.md5 $TVFILE && sync) || fatal 'Extract root'
    osum=$(cat $TRIMG.md5)
